define(['exports'], (exports) => {
      ...
      (_extends = Object.assign
        ...
        : function (n) {
          for (let e = 1; e < arguments.length; e++) {
            const t = arguments[e]
            for (const r in t) Object.hasOwn(t, r) && (n[r] = t[r])
          }
          return n
        }),
  ...
  const getIdbProxyableTypes = () => idbProxyableTypes
    || (idbProxyableTypes = [
      IDBDatabase,
      IDBObjectStore,
      IDBIndex,
  ...
  // This is a function to prevent it throwing up in node environments.
  const getCursorAdvanceMethods = () => cursorAdvanceMethods
  ...
  const cacheDonePromiseForTransaction = (tx) => {
   ...
   */
  const openDB = (name, version, { blocked, upgrade, blocking, terminated } = {}) => {
  ...
  /*
      ...
      https://opensource.org/licenses/MIT.
    */
  const stripParams = (fullURL, ignoreParams) => {
   ...
   */
  const cacheMatchIgnoreParams = async (cache, request, ignoreParams, matchOptions) => {
   ...
   * @private
   */
  const _nestedGroup = (groupTitle, urls) => {
    if (urls.length === 0) {
      return
    }
    logger.groupCollapsed(groupTitle)
   ...
   * @private
   */
  const canConstructResponseFromBodyStream = () => {
  ...
  PrecacheStrategy.copyRedirectedCacheableResponsesPlugin = {
    cacheWillUpdate: async ({ response }) => response.redirected ? await copyResponse(response) : response,
  }
...
// # sourceMappingURL=workbox-3503af68.js.map
